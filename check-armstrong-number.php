<html>
<head>
	<title>Check Armstrong Number - Simple Java Algorithms | Codesena</title>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<meta name="author" content="Codesena">
		<meta name="description" content="Armstrong Number: A number which is equal to the sum of its digits raise to the power total number of digits in that number is called as Armstrong number.">
        <meta name="keywords" content="Java, Algorithms, Armstrong Numbers, Simple Program, Beginner level, Codesena">
	...
	<div id="fb-root"></div>
		...
		<div class="container-fluid">
			<div class="row-fluid">
				...
				<div id="previous" class="col-md-2 col-lg-2 pull-left" onclick="location.href='check-prime-number.php'">Previous <i class="fa fa-chevron-left pull-left" aria-hidden="true"></i>
				</div>

				<div id="next" class="col-md-2 col-lg-2 pull-right" onclick="location.href='check-palindrome.php'">Next <i class="fa fa-chevron-right pull-right" aria-hidden="true"></i>
				</div>
				</div>
				<div id="middle">
				...
				<hr>
				<h3 id="title">Program to check whether the given number is Armstrong or not.</h3>
				<hr>
				</div>
				<div id="program" class = "row-fluid">
				<p>
				This program will determine whether the integer value we are providing to the
				function is an Armstrong number or not and display the output in console.</p>
				<p>Let's understand what are Armstrong numbers in detail.</p>
				<p><b>Armstrong Numbers :</b> A number which is equal to the sum of its digits raise to the power total number of digits in that number is called as Armstrong number. 
				</p>
				<p>Consider a number 371. It has three digits, i.e. 3, 7 and 1. So, according to the definition, 371 is an Armstrong number if :<br>
				<mark>371 = (3^3) + (7^3) + (1^3)</mark><br>
				Let's check if it is true.<br>
				<mark>(3^3) + (7^3) + (1^3) = 27 + 343 + 1 = 371</mark> <br>
				Hence, the result shows 371 is an Armstrong number. Other examples of Armstrong numbers are 153, 407, 1634, 8208 etc.
				</p>				 
				<p>
				In the below program, we have implemented the logic explained above. We can calculate the power of a digit either by multiplying the digit with itself the number of times the total digits in given number or by using <b>Math.pow(double base, double power)</b> function of Java.
				</p>
<pre class="language-java line-numbers" >
<code>package com.algorithms.beginner;

/**
 * The ArmstrongNumber program implements an application that checks whether the
 * integer input we are giving to the function is an Armstrong integer or not
...
public class ArmstrongNumber {
	...
	// method to check whether integer is armstrong or not
	public void isArmstrong(int number) {
		...
		while (temp != 0) {
			rem = temp % 10;
			num = num + rem * rem * rem; // or we can use num = (int) (num + Math.pow(rem, 3));
			temp = temp / 10;
		}
		if (num == number) {
	...
	public static void main(String[] args) {
		...
		obj.isArmstrong(153); // prints 153 is an Armstrong Number.
		obj.isArmstrong(253); // prints 253 is not an Armstrong Number.
	}
}
</code>
</pre>
...
<p>Comment your queries or give feedback :</p>
	<div id="comment-pane" class="row-fluid">
			...
			<form method="post">
				...
				<button id="submit-comment" type="submit" name="submit_comment" class="btn btn-primary pull-left">Comment <span><i class="fa fa-comments fa-fw"></i></span></button>
   				...
   				<?php
					...
					mysqli_query($conn,"INSERT INTO comment_reply_tab(page_id, user_name, email_id, comment, timings, parent_id) VALUES('armstrong-number', '$name', '$email', '$comment', NOW(),0)"); 
				...
				<?php
				 ...
				 if(isset($_POST["submit_reply"]))
					...
					//change page_id for other pages
					mysqli_query($conn,"INSERT INTO comment_reply_tab(page_id, user_name, email_id, comment, timings, parent_id) VALUES('armstrong-number', '$replier_name', '$replier_email', '$replier_comment', NOW(),$parent_id)"); 
		...
		<?php
			...
			//change page_id for other pages
			$sql = "SELECT comment_id, page_id, user_name, email_id, comment, timings, parent_id FROM comment_reply_tab where parent_id = 0 and page_id = 'armstrong-number' ";
