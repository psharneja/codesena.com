<html>
<head>
	<title>Sum of the product of all possible subsets | Codesena</title>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<meta name="author" content="Codesena">
		<meta name="description" content="Here is the  program to print the sum of the product of all the possible subsets of an array. To understand this program, you need to do a little math work. ">
        <meta name="keywords" content="Java, Algorithms, Program, subset, sum of product, Beginner level, Codesena">
	...
	<div id="fb-root"></div>
		...
		<div class="container-fluid">
			<div class="row-fluid">
				...
				<div id="previous" class="col-md-2 col-lg-2 pull-left" onclick="location.href='print-pascal-triangle.php'">Previous <i class="fa fa-chevron-left pull-left" aria-hidden="true"></i>
				</div>

				<div id="next" class="col-md-2 col-lg-2 pull-right" onclick="location.href='add-two-matrices.php'">Next <i class="fa fa-chevron-right pull-right" aria-hidden="true"></i>
				</div>
				</div>
				<div id="middle">
				...
				<hr>
				<h3 id="title">Program to print the sum of the product of all the possible subsets of an array.</h3>
				<hr>
				</div>
				<div id="program" class = "row-fluid">
				<p> Let's say we have an array with three elements { X, Y, Z }.	<br>
				All the possible subsets of given array are <mark>{X}, {Y}, {Z}, {X,Y}, {X,Z}, {Y,Z}, {X,Y,Z}</mark>.
				To understand this program, you need to do a little mathematical work. </p>
				<p>
				Initially, let Result3 be the sum of the product of all subsets of { X, Y, Z }. <br>
				Therefore, <mark>Result3 = X + Y + Z + X * Y + X * Z + Y * Z + X * Y * Z</mark><br>
				After reducing the above equation,<br>
                <mark>Result3 = X + Y + X * Y + Z * (1 + X + Y + X * Y)</mark><br>
				</p>
				<p>
				Now, let Result2 = X + Y + X * Y. Now, the equation becomes :<br>
				<mark>Result3 = Result2 + Z * (1 + Result2)</mark><br>
				</p>
				<p>
				In the same manner, reducing the Result2 :<br>
				Result2 = X + Y * (1 + X)<br>
				Result1 = X<br>
				Now the equation we get : <br>
				Result2 = Result1 + Y * (1 + Result1)<br>
				</p>
				<p>
				We can summarize the problem as,<br>
				Result1 = X<br>
				Result2 = Result1 + Y * (1 + Result1)<br>
				Result3 = Result2 + Z * (1 + Result2)<br>
				</p>
				<p>
				Same logic, we have implemented in our code.
				</p>			
<pre class="language-java line-numbers" >
<code>package com.algorithms.intermediate;

/**
 * The SubsetsProblem program will print the sum of product of all the possible
 * subsets of an array.
 *
 * @author Pawan
 * @version 1.0
...
public class SubsetsProblem {
    ...
    static long sopOfSubsets(int arr[]) {
        long result = 0;
        for (int element : arr) {
            result = result + element * (result + 1);
        }
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        int array[] = { 2, 3, 5 };
        long sop = sopOfSubsets(array);
        System.out.println("The sum of product of all possible subsets of given array is : "+sop);
    }
}
</code>
</pre>
...
<code>
The sum of product of all possible subsets of given array is : 71
</code>
</pre>
<p>Comment your queries or give feedback :</p>
	<div id="comment-pane" class="row-fluid">
			...
			<form method="post">
				...
				<button id="submit-comment" type="submit" name="submit_comment" class="btn btn-primary pull-left">Comment <span><i class="fa fa-comments fa-fw"></i></span></button>
   				...
   				<?php
					...
					mysqli_query($conn,"INSERT INTO comment_reply_tab(page_id, user_name, email_id, comment, timings, parent_id) VALUES('sop-subsets', '$name', '$email', '$comment', NOW(),0)"); 
				...
				<?php
				 ...
				 if(isset($_POST["submit_reply"]))
					...
					//change page_id for other pages
					mysqli_query($conn,"INSERT INTO comment_reply_tab(page_id, user_name, email_id, comment, timings, parent_id) VALUES('sop-subsets', '$replier_name', '$replier_email', '$replier_comment', NOW(),$parent_id)"); 
		...
		<?php
			...
			$sql = "SELECT comment_id, page_id, user_name, email_id, comment, timings, parent_id FROM comment_reply_tab where parent_id = 0 and page_id = 'sop-subsets' ";
