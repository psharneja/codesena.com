<html>
<head>
	<title>Check Pangrams - Algorithms in Java | Codesena</title>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<meta name="author" content="Codesena">
		<meta name="description" content="The Pangram is defined as a sentence that contains every letter of the alphabet at least once. Pangram sentence is also known as Holoalphabetic sentence.">
        <meta name="keywords" content="Java, Algorithms, Check Pangram, Intermediate, Program, Beginner level, Codesena">
	...
	<div id="fb-root"></div>
		...
		<div class="container-fluid">
			<div class="row-fluid">
				...
				<div id="previous" class="col-md-2 col-lg-2 pull-left" onclick="location.href='check-anagrams.php'">Previous <i class="fa fa-chevron-left pull-left" aria-hidden="true"></i>
				</div>

				<div id="next" class="col-md-2 col-lg-2 pull-right" onclick="location.href='print-pascal-triangle.php'">Next <i class="fa fa-chevron-right pull-right" aria-hidden="true"></i>
				</div>
				</div>
				<div id="middle">
				...
				<hr>
				<h3 id="title">Program to check if the input strings are Pangrams or not.</h3>
				<hr>
				</div>
				<div id="program" class = "row-fluid">
				<p>What are Pangrams? <br>
				<b>Pangram :</b> The Pangram is defined as a sentence that contains every letter of the alphabet at least once. Pangram sentence is also known as <b>Holoalphabetic</b> sentence. For example, "The quick brown fox jumps over the lazy dog".<br>
			    The program to check if the string is a pangram or not involves the following steps:<br>
				<ol>
				<li>Firstly check that the minimum length of the input string should be greater than or equal to 26. If the length is greater than or equal to 26, go to the next step.</li>
				<li>In the second step, convert the input string to lower case and store it in a temporary variable.</li>
				<li>Now check if the sentence stored in the temporary variable contains all the letters from a to z using <b>indexOf()</b> method of String class in Java.</li>
				</ol>	
                <p>The indexOf() method returns the index of the given character in a string. If the character is not found, it will
				return a negative value. See the below implementation.		
				</p>
...
/**
 * The Pangram program checks if the input string is pangram or not.
 *
 * @author Pawan
 * @version 1.0
 * @since 2017-05-24
...
public class Pangram {
	...
	private static final int TOTAL_LETTERS_FROM_ATOZ = 26;
	...
	public void checkPangram(String str) {
		...
		if (str.length() < TOTAL_LETTERS_FROM_ATOZ) {
			isPangram = false;
		} else {
			String tempStr = str.toLowerCase(); // convert to lowercase
		...
		if (isPangram) {
			System.out.println(str + " --> Pangram");
		} else {
	...
	public static void main(String[] args) {
		...
		pangram.checkPangram("The quick brown fox jumps over the lazy dog");
		pangram.checkPangram("Pack my box with five dozen liquor jugs");
		pangram.checkPangram("My name is Pawan");
	}
}
</code>
</pre>
...
My name is Pawan --> Not a Pangram
</code>
</pre>
<p>Comment your queries or give feedback :</p>
	<div id="comment-pane" class="row-fluid">
			...
			<form method="post">
				...
				<button id="submit-comment" type="submit" name="submit_comment" class="btn btn-primary pull-left">Comment <span><i class="fa fa-comments fa-fw"></i></span></button>
   				...
   				<?php
					...
					mysqli_query($conn,"INSERT INTO comment_reply_tab(page_id, user_name, email_id, comment, timings, parent_id) VALUES('check-pangrams', '$name', '$email', '$comment', NOW(),0)"); 
				...
				<?php
				 ...
				 if(isset($_POST["submit_reply"]))
					...
					//change page_id for other pages
					mysqli_query($conn,"INSERT INTO comment_reply_tab(page_id, user_name, email_id, comment, timings, parent_id) VALUES('check-pangrams', '$replier_name', '$replier_email', '$replier_comment', NOW(),$parent_id)"); 
		...
		<?php
			...
			$sql = "SELECT comment_id, page_id, user_name, email_id, comment, timings, parent_id FROM comment_reply_tab where parent_id = 0 and page_id = 'check-pangrams' ";
